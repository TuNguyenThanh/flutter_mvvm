...
- **Seed color:** `#ffb43f` (used to derive `ColorScheme` via `ColorScheme.fromSeed` where applicable).
...
Primary family: **Quicksand**

- **Quicksand** — 5 file(s) configured in `pubspec.yaml` (app-wide `fontFamily` when primary)
...
Import tokens via `package:flutter_mvvm/src/theme/theme_constants.dart`.
